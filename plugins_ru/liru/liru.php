<?php
/*
Plugin Name: Import from LiveInternet.ru
Plugin Slug: liru_Import
Plugin URI: http://blog.dmpink.ru/liru
Description: Импорт записей из XML файла LiveInternet.ru в WordPress.
...
function import_posts() {
	...
	foreach ($posts as $post) {
	  ...
	  $post_title = $wpdb->escape(trim($post_title));
		...
		$guid = str_replace(array ('<![CDATA[', ']]>'), '', trim($guid[1]));
		$post_name='';
		$arr=split('/',$guid);
		$arr_cnt=count($arr);
		if ($arr_cnt>0){
			if ($arr[$arr_cnt-1]!=''){
				$post_name=$arr[$arr_cnt-1];}
			else{
				$post_name=$arr[$arr_cnt-2];}
		}
		// Clean up content
    ...
    $tags_input = iconv("windows-1251","utf-8",$tags_input);
		...
		if ($post_id = post_exists($post_title, $post_content, $post_date)) {
			...
			$postdata = compact('post_author', 'post_date', 'post_content', 'post_title', 'post_status', 'tags_input', 'guid', 'post_name');
...
$GLOBALS['wp_importer'] = new WP_Importer();
